...
public class addGoodsActionDAO {
	public static int addGoodsAction(String category, String empId, String goodsTitle, String filename, 
			String goodsContent, int goodsPrice) throws Exception {
		
		int row = 0;
		
		Connection conn = DBHelper.getConnection();
		String sql = "insert into goods(category, emp_id, goods_title, filename, goods_content, goods_price, update_date, create_date)"
				+ " values(?, ?, ?, ?, ?, ?, now(), now())";
